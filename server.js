...
const JWT_SECRET = process.env.JWT_SECRET || "forge-change-me-in-production";
...
app.get("/api/health", async (req,res)=>{
  try {
    ...
    res.json({ ok: true, service: "FORGE", time: new Date().toISOString(), database: "ok" });
  } catch (error) {
    res.status(503).json({ ok: false, service: "FORGE", error: "Database unavailable", detail: error.message });
  }
});
app.get(/^(?!\/api\/).*$/, (req,res)=>res.sendFile(path.join(__dirname,"public","index.html")));

app.listen(PORT, ()=>console.log(`FORGE running on http://localhost:${PORT}`));
