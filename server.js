...
app.get("/api/health", async (req,res)=>{
  try {
    ...
    return res.json({ ok: true, service: "FORGE", time: new Date().toISOString(), database: "ok" });
  } catch (error) {
    return res.json({ ok: true, service: "FORGE", time: new Date().toISOString(), database: "unavailable", detail: error.message });
  }
});
